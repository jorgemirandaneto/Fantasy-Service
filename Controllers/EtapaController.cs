...
namespace FantasyServer.Controllers {
    ...
    public class EtapaController : ControllerBase {
        ...
        public IEnumerable<EtapaParticipante> getNota (int idEtapa) {
            try {
                var NotasEtapas = (from nota in _context.EtapaParticipantes where (nota.fk_etapa == idEtapa) select new EtapaParticipante {
                    id = nota.id,
                        nota = nota.nota,
                        ano = nota.ano,
                        fk_etapa = nota.fk_etapa,
                        fk_participante = nota.fk_participante,
                        Etapas = (from e in _context.Etapas where e.id == nota.fk_etapa select new Etapa {
                            id = e.id,
                                nome = e.nome
                        }),
                        Participantes = (from p in _context.Participantes where p.id == nota.fk_participante select new Participante {
                            id = p.id,
                                nome = p.nome,
                                email = p.email
                        })
        ...
        [HttpGet, Route("/EtapaFinalizada/{idEtapa}")]
        public bool getEtapaFinalizada (int idEtapa) {
            var queryEtapa = (from devedores in this._context.Devedores
                                where devedores.fk_etapa_devedores == idEtapa
                                select new Devedores {
                                    id = devedores.id
                                }
            ).Distinct().ToList();

            return queryEtapa.Count > 0 ? true : false;
        }        
    } 
}